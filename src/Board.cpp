...
/* Used on multithread perft function only */
typedef struct {
    Board board;
...
void menu(int& menuorboard) {
    ...
    */
    ...
    float sizeOftext2 = screenHeight/20;
    ...
    if (CheckCollisionPointRec(mousePoint,rec5)) {
        DrawText("clique esc para sair", xOfButtons, screenHeight/3, sizeOftext2, WHITE);
...
void BoardInit(Board &board) {
    ...
    std::memset(&board.movingPiece, 0, sizeof board.movingPiece);
    std::memset(&board.move, 0, sizeof board.movingPiece);
    ...
    for (; fen[i] != ' '; i++) {
        if (fen[i] == '/') {
            ...
            for (int j = 0; j < 12; j += 2) {
                ...
                board.squares[PieceSquare(rank, file)] = pieces_data[j + 1] + (std::isupper(fen[i]) ? PIECE_WHITE : PIECE_BLACK);
                file++;
...
#define THREADS_NUMBER 6
    ...
    for (int move = 0; move < 4096; ) {
        for (thr = 0; thr < THREADS_NUMBER && move < 4096; thr++, move++) {
            std::memcpy(&datas[thr].board, &board, sizeof(board));
    ...
    if (board.state.waitPromotion) {
        ...
        if (!board.state.waitPromotion) {
            if(board.state.whoMoves == PIECE_BLACK) {
                saxa_move saxaMove = finalMoveGrade(board,saxaDepht);
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (CheckCollisionPointRec(GetMousePosition(), btnRectangle)) {
                ...
                if (clickOption == 0 && IsMouseButtonDown(MOUSE_LEFT_BUTTON)) {
                    ...
                    if (clickOption == i + 1 && clickOption <= 2) {
                        if (clickOption == 2)
                            BoardSaveFEN(board);
                        else
                            BoardInit(board);
...
#ifdef BOARD_PERFT
void perft(PerftData& data) {
    PerftData copiedData;

    if (data.depth == 1) {
        data.nodes = data.board.move.count;
        return;
    }
    ...
    for (int from = 0; from < 64; from++) {
        for (int to = 0; to < 64; to++) {
            std::memcpy(&copiedData, &data, sizeof(data));
            copiedData.nodes = 0;

            if (!BoardMakeMove(copiedData.board, from, to, true))
                continue;

            perft(copiedData);
            data.nodes += copiedData.nodes;
        }
    }
    data.depth++;
}
